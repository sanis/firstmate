...
Cross-home reads validate the seeded identity and operational-directory boundaries, use per-home time and output bounds, and classify unavailable, malformed, or untrustworthy structured state as unknown rather than reviving a parent event as current work.
...
A backlog-vs-metadata inventory mismatch keeps those same surfaces and the home's own current classification, because it is a repair notice about that home's own books rather than a reason to distrust what the snapshot projected separately; every other invalid path stays strict and exposes none of those child-derived surfaces.
...
A presence-gated sub-supervisor (`bin/fm-supervise-daemon.sh`) extends this for walk-away supervision: the `/afk` skill enters through `bin/fm-afk-launch.sh`, which owns where the daemon is hosted - by backend first, then harness, because a backend whose native agent state observes a pane's own background jobs cannot host the daemon in the pane it must deliver into - and runs the shared foreground entry `bin/fm-afk-start.sh` there, after which the watcher reverts to daemon-managed one-shot mode and the daemon self-handles routine wakes in bash.
