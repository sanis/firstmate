...
| `fm-landed-lib.sh`       | Shared "what belongs in Recently Landed" selector for both snapshot projections      |
...
| `fm-remote-herdr-guard.sh` | launchd exec target that makes the remote account's Aqua login session own the fm-remote Herdr server |
| `fm-remote-herdr-owner-lib.sh` | Shared discovery of who owns a Herdr session socket and whether that process was born in the Aqua login session |
...
| `fm-evidence-rule-lib.sh` | Shared evidence-delivery rule text for ship briefs and promotion handoffs, pointing at the `evidence-artifacts` owner |
| `fm-herdr-lab.sh`        | Provision and guardedly operate an isolated, never-default Herdr lab session         |
| `fm-download-lib.sh`     | Single owner of the installers' retrying release-asset download: what is worth retrying, and the bound on a hopeless one |
