...
The arming, poll, and missing-`glab` evidence through the GitHub-unaffected case was collected on 2026-07-21, except for the three `fm-pr-check.sh` transcripts, which were re-run after that command gained its description check.
Those three carry no single date: each is marked where it appears with the date it actually ran and the versions it ran under.
"Merging a merge request" was run on 2026-08-22.
