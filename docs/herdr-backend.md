...
A fresh launch performs no separate delivery-path verification: the daemon validates its own backend and supervisor target at startup and exits when either fails, which fails the launch through the launcher's readiness wait.
Verification runs when a daemon is ALREADY alive, which is the only place a pre-existing in-pane daemon can be caught, since such a daemon is never re-created.
