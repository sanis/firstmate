...
# the evidence rule, and - rendered from bin/fm-dod-lib.sh, the single owner an
# ordinary ship brief also uses - the
...
# A promoted worker keeps its scout brief, which carries no delivery rules, so
# these instructions are also the only place the evidence contract reaches it
# before it writes a PR or MR body; that sentence comes from
# bin/fm-evidence-rule-lib.sh, shared with the ship brief bin/fm-brief.sh
...
#
# The evidence rule is appended after upstream's last numbered item rather than
# inserted among them, so upstream's own numbering is left untouched - do not
# renumber it into the middle of that list. The Definition of done appended below
# says "escalate to firstmate using rule 6's ask-user format" and "rule 7 owns the
# checks"; both address the worker's own brief Rules list - the same list the
# delivery contract below calls "the scout rule 6 escalation shape" - and not this
# spec's numbering. The weakness worth recording is that those pointers cross-
# reference a list POSITION in a document both upstream and this fork insert into,
# so a future insertion on either side can silently re-aim them with no test or lint
# to catch it. That redesign is upstream's to make and is out of scope here.
