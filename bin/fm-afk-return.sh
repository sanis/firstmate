...
# and `check` succeeds. A wake drain that exits zero while printing
# `WAKE DRAIN SKIPPED:` held the gate too: the queue lock was held, so exit
# status alone cannot be read as a drained queue. Repeated begin/check calls are idempotent. `guard` and
