...
When the captain invokes `/clickup`, load the `clickup` skill; it owns the main-session-only ClickUp connector pipeline that pulls one eligible ClickUp task through intake, interview, and delegation as a normal ship task.
...
It also refuses, recording nothing, when the description hands the reader a local filesystem path instead of uploaded evidence; steer the worker to deliver that evidence through `evidence-artifacts` and rerun, rather than bypassing the refusal.
Tell the captain the PR's full `https://...` URL copied from the worker's ready line or the task's `pr=` metadata, always the complete link rather than a bare `#number`, a concise outcome summary, and the no-mistakes risk level when applicable.
...
When a ClickUp-linked task - one whose backlog record carries a `clickup:` line - reports its PR checks green or its PR merged, load the `clickup` skill and post the matching ClickUp update before reporting or cleanup.
...
- `evidence-artifacts` - load before putting a screenshot or other evidence file into a pull request, a merge request, or a ClickUp update; a local file path is never the delivered form.
