...
That judgement is delegated to the pinned shared `require-no-mistakes` action, which binds the attestation to the current PR head commit and requires the `review`, `test`, and `document` steps to each read `completed`, so a stale attestation, a missing `head_sha`, or a skipped required step fails.
A quota skip or an agent skip on one of those steps is deliberately not compliant.
A body carrying the signature is decided on that body alone, so a signature whose attestation is missing, unparseable, or bound to another head fails there rather than falling through to the commit proof below.
...
3. Initialize the gate with your fork as the push target: `no-mistakes init --fork-url git@github.com:<you>/firstmate.git` (contributing to firstmate requires **no-mistakes v1.46.0+** for the structured attestation; without a fork, plain `no-mistakes init` still works for maintainers with push access).
...
`.no-mistakes.yaml` publishes test evidence to the orphan `no-mistakes/evidence` branch, which shares no history with code branches, pins the gate's lint command to `bin/fm-lint.sh`, matching the Linux CI lint job, and pins its test command to the runner's changed-file path with the live-Herdr family excluded, exactly as [`docs/configuration.md`](docs/configuration.md) ("Gate defaults") records.
